...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "Never finish on an unsigned or pending card. If you handed over a "
        "sign_url, you CANNOT see the result: say so, ask the user to tell you "
        "once they have signed, and then confirm it with "
        "get_status(history_limit=5) — the order's status and tx_hash appear "
        "there. Do not go quiet after dropping a link, and never leave the user "
        "to guess whether it landed. Once a tx_hash exists, "
